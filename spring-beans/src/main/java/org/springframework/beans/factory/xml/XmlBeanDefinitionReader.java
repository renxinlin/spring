...
public class XmlBeanDefinitionReader extends AbstractBeanDefinitionReader {
	...
	protected int doLoadBeanDefinitions(InputSource inputSource, Resource resource)
			throws BeanDefinitionStoreException {
		try {
			// 从Resource 转换成inputstream 转换成InputSource 转换成Document
			// 通过Document去解析bean,完成bean工厂的任务
			/**
			 * 过程如下
			 * 装资源文件 后首先对 Resource 使用ncodedResource 类进行封装
			 * 2. 获取输入流Resource 巾获取对应的 InputStrearn 并构造 lnputSource
			 * 3. 委托实现documentReader.registerBeanDefinitions(
			 */
	...
	public int registerBeanDefinitions(Document doc, Resource resource) throws BeanDefinitionStoreException {
		...
		// 注册BD DefaultBeanDefinitionDocumentReader实现
		documentReader.registerBeanDefinitions(doc, createReaderContext(resource));
		// getRegistry 就是xmlBeanFactory 注意spring的套路 可以对程序员提供的上下文或者bean工厂
		// 注册器一般就是context 也是 registry 也是beanfactory
